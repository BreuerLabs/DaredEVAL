        ...
        def __init__(self, config):
            ...
            if 'resnet' in arch:
                ...
                if arch == 'resnet18':
                    ...
                    skip_defended_model = resnet.resnet34(weights=weights, skip=config.defense.skip)
                elif arch == 'resnet50':
                    weights = resnet.ResNet50_Weights.DEFAULT if pretrained else None
                    skip_defended_model = resnet.resnet50(weights=weights, skip=config.defense.skip)
                elif arch == 'resnet101':
                    weights = resnet.ResNet101_Weights.DEFAULT if pretrained else None
                    skip_defended_model = resnet.resnet101(weights=weights, skip=config.defense.skip)
                elif arch == 'resnet152':    
            ...
            #         skip_defended_model = densenet.densenet121(weights=weights, skip=config.defense.skip)
            #     elif arch == 'densenet161':
            #         weights = densenet.DenseNet161_Weights.DEFAULT if pretrained else None
            #         skip_defended_model = densenet.densenet161(weights=weights, skip=config.defense.skip)
            #     elif arch == 'densenet169':
            #         weights = densenet.DenseNet169_Weights.DEFAULT if pretrained else None
            #         skip_defended_model = densenet.densenet169(weights=weights, skip=config.defense.skip)
            #     elif arch == 'densenet201':
            #         weights = densenet.DenseNet201_Weights.DEFAULT if pretrained else None
            #         skip_defended_model = densenet.densenet201(weights=weights, skip=config.defense.skip)
                
            #     skip_defended_model.classifier = self.model.classifier

            elif 'resnext' in arch:
                if arch == 'resnext50':
                    weights = resnet.ResNeXt50_32X4D_Weights.DEFAULT if pretrained else None
                    skip_defended_model = resnet.resnext50_32x4d(weights=weights, skip=config.defense.skip)
                elif arch == 'resnext101':
                    weights = resnet.ResNeXt101_32X8D_Weights.DEFAULT if pretrained else None
                    skip_defended_model = resnet.resnext101_32x8d(weights=weights, skip=config.defense.skip)
