...
class Discriminator(nn.Module):
    def __init__(self, config):
        ...
        self.label_emb = nn.Embedding(config.dataset.n_classes, config.attack.label_dim)
        
        # Unpack the hyperparameter dict
        hyper = config.attack.discriminators
        
        # Model architecture with parameterized layer sizes and dropout rate
        layers = [
                nn.Linear(img_dim + config.attack.label_dim, hyper.hidden_dim[0]), 
                nn.LeakyReLU(hyper.negative_slope, inplace=True), 
                nn.Dropout(hyper.dropout_rate)
                ]
        ...
        for i in range(1, len(hyper.hidden_dim)):
            ...
            layers.append(nn.Dropout(hyper.dropout_rate))
...
class Generator(nn.Module):
    def __init__(self, config):
        ...
        layers = [nn.Linear(config.attack.latent_dim + config.attack.label_dim, hyper.hidden_dim[0]), nn.LeakyReLU(hyper.negative_slope, inplace=True)]
...
class GAN:
    def __init__(self, config):
        ...
        # Optimizers
        self.g_optimizer = torch.optim.Adam(self.generator.parameters(), lr=config.attack.generator.hyper.lr)
        self.d_optimizer = torch.optim.Adam(self.discriminator.parameters(), lr=config.attack.discriminator.hyper.lr)
    ...
    def generator_train_step(self):
        ...
        fake_labels = Variable(torch.LongTensor(np.random.randint(0, self.num_classes, self.batch_size))).to(self.device)
    ...
    def discriminator_train_step(self, real_images, labels):
        ...
        fake_labels = Variable(torch.LongTensor(np.random.randint(0, self.num_classes, real_images.size(0)))).to(self.device)
        fake_images = self.generator(z, fake_labels)
    ...
    def train(self, data_loader):
        all_d_loss = []
        for epoch in range(self.config.attack.hyper.epochs):
            ...
            for i, (images, labels) in enumerate(tqdm(data_loader)):
                ...
                d_loss = 0
                for _ in range(self.attack.hyper.n_critic):
                    d_loss = self.discriminator_train_step(real_images, labels)

                # Train generator
                g_loss = self.generator_train_step()

                # Display step for logging and generating sample images
                if (epoch * len(data_loader) + i + 1) % self.training.display_step == 0:
                    print(f'Epoch [{epoch+1}/{self.config.training.epochs}], Step [{i+1}/{len(data_loader)}], d_loss: {d_loss:.4f}, g_loss: {g_loss:.4f}')
